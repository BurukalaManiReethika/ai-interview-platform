...
from services.ats_service import calculate_ats_score, extract_skills, missing_skills
from services.question_service import generate_interview_questions
...
"""
    ...
    sample_question = "Explain the difference between Abstraction and Encapsulation."
    return render_template("mock_interview.html", question=sample_question)
...
def leaderboard():
    ...
    for user in users:
        ...
        avg = 0
        if resumes:
            avg = sum(r.ats_score for r in resumes) / len(resumes)
        leaderboard_data.append({"name": user.name, "score": round(avg)})
...
@login_required
def download_report():
    path = create_report("report.pdf", 85, ["Python", "Flask"], "Sample Questions")
...
def admin():
    ...
    return render_template("admin.html", users=users, resumes=resumes, interviews=interviews)
